<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Tambah Transaksi</h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                ...
                <form action="{{ route('transactions.store') }}" method="POST" class="space-y-6">
                    ...
                    <!-- Akun -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Akun</label>
                        <select name="account_id" required 
                                class="w-full mt-1 p-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            @if ($accounts->isEmpty())
                                <option value="" disabled selected>👀 Hmm... belum ada akun nih!</option>
                            @else
                                <option value="" disabled selected>🔍 Pilih akun keuangan...</option>
                                @foreach ($accounts as $account)
                                <option value="{{ $account->id }}">{{ $account->name }}</option>
                                @endforeach
                            @endif
                        </select>
                        <!-- Navigasi ke Halaman Accounts -->
                        <div class="mt-2">
