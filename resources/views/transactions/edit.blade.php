<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Transaksi</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                ...
                <!-- Tombol Back -->
                <div class="mb-6">
                    <a href="{{ route('transactions.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-blue-700 bg-blue-100 hover:bg-blue-200 rounded-lg border border-blue-300">
                 ...
                 <!-- Ganti Gambar Header jadi Background Gradasi dengan font Copperplate -->
                ...
                <form action="{{ route('transactions.update', $transaction->id) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label for="transaction_date" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Tanggal Transaksi</label>
                            <input type="date" id="transaction_date" name="transaction_date" value="{{ old('transaction_date', \Carbon\Carbon::parse($transaction->transaction_date)->toDateString()) }}" 
       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-blue-500" required>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200" required>{{ old('description', $transaction->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="type" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Tipe Transaksi</label>
                            <select id="type" name="type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 focus:border-blue-500" required>
                        ...
                        <div>
                            ...
                            <select id="category_id" name="category_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 focus:ring-blue-500 focus:border-blue-500" required>
                                @if ($categories->isEmpty())
                                    <option value="" disabled selected>👀 Hmm... belum ada kategori nih!</option>
                                @else
                        ...
                        <div>
                            <label for="account_id" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Akun</label>
                            <select id="account_id" name="account_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600  dark:text-gray-200 focus:ring-blue-500 focus:border-blue-500" required>
                                
                                @if ($accounts->isEmpty())
                                    <option value="" disabled selected>👀 Hmm... belum ada akun nih!</option>
                                @else
    ...
    amountInput.addEventListener('input', function (e) {
        let value = e.target.value.replace(/\./g, ''); // Hapus titik yang sudah ada
        let formattedValue = new Intl.NumberFormat('id-ID').format(value); // Format ulang
        e.target.value = formattedValue;
    });

    // Sebelum form disubmit, pastikan kita menghapus titik di amount
